...
int i=0;  
    while(i<s1.size())  
    {
        ...
        if(s1[i]!='a'){i++;continue;}
        if(s1.size()>510 ){bol=false;break;}
        string temp;
        for(int j=0;j<i;j++)temp+=s1[j];
        temp+=s2;
        for(int j=i+1;j<s1.size();j++)temp+=s1[j];
        s1=temp; 
        //cout<<s1<<endl;
        mp[s1]++;
        //cout<<temp<<endl;
    ...
    //for(auto u:mp)cout<<u.first<<endl;

    if(bol)cout<<mp.size()<<endl;
    else cout<<-1<<endl;
}
//...........<<<<<<<<<<end>>>>>>>>>>>...............
